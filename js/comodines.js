...
for (let i = 1; i <= 150; i++) {
    ...
    const carta = {
        img: "img/comodines/comodin" + i + ".webp",
        nombre: "Elemento " + i,
        categoria: "comun", // SIEMPRE definido
        desc: "Descripción " + i
    };

    if (datos[i]) {
        Object.assign(carta, datos[i]);
    }
...
function renderPagina() {
    ...
    const totalPaginas = Math.ceil(cartas.length / porPagina);
...
function mostrarTooltip(e, carta, elemento) {
    ...
    if (!tooltip) return; // 👈 evita bug si no existe
...
function crearTextoOla(texto) {
    ...
    return `<span class="wave">` +
        limpio.split("").map(l => `<span>${l}</span>`).join("") +
    `</span>`;
}

/* ===== TÍTULO ANIMADO ===== */
function crearTituloAnimado(texto) {
    return `<span class="titulo-animado">` +
        texto.split("").map(l => {
            if (l === " ") return `<span>&nbsp;</span>`; // 👈 clave
            return `<span>${l}</span>`;
        }).join("") +
    `</span>`;
}
